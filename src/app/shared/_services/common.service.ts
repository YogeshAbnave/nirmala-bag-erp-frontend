...
export class  CommonService {
  ...
  constructor( private httpClient: HttpClient, private _httpService: HttpService, private toastr: ToastrService, private router: Router) { }
  ...
  setSession(sessionData: any) {
    window.localStorage.setItem("nector-farm-session", this.encode(JSON.stringify(sessionData)));
  }
  ...
  getSession() {
    return JSON.parse(this.decode(window.localStorage.getItem("nector-farm-session")));
  }
  ...
  deleteSession() {
    localStorage.removeItem("nector-farm-session");
